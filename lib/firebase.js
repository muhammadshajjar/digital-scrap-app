...
export const setPickupSchedule = async (pickupData) => {
  try {
    await firestore().collection("schedule").doc().set(pickupData);
  } catch (e) {
    throw new Error(e.message);
  }
};

export const getRiderBasedOnAreaAssigned = async (pickupCoords) => {
  try {
    const result = await firestore().collection("ridersprofile").get();
    const ridersProfileData = result?.docs
      ?.map((rider) => ({
        id: rider.id,
        data: rider.data(),
      }))
      .filter((rider) => rider?.data?.assignedArea?.coordinates);
