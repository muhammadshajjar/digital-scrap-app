...
import { isPointInsidePolygon } from "../helper/utilityFunctions";
...
export const setPickupSchedule = async (pickupData) => {
  try {
    const riderFound = await getRiderBasedOnAreaAssigned([
      pickupData?.lat,
      pickupData?.lng,
    ]);
    ...
    if (riderFound) {
      await firestore()
        .collection("schedule")
        .doc()
        .set({ ...pickupData, riderId: riderFound?.id });
    } else {
      throw new Error("Unable to find rider in your area!");
    }
  } catch (e) {
...
const getRiderBasedOnAreaAssigned = async (pickupCoords) => {
  try {
    const result = await firestore().collection("ridersprofile").get();
    const ridersProfileData = result?.docs?.map((rider) => ({
      id: rider.id,
      data: rider.data(),
    }));

    return ridersProfileData.find((rider) => {
      const result = isPointInsidePolygon(
        pickupCoords,
        JSON.parse(rider?.data?.assignedArea?.coordinates)
      );

      if (result) return rider;
    });
  } catch (e) {
    throw new Error("Failed to fetch rider profile: " + error.message);
  }
...
export const getAllSchedules = async (scheduleData) => {
  const { type, id } = scheduleData;
  console.log(type, id);
  try {
    ...
    const scheduleData = result?.docs?.map((schedule) => {
      return {
        ...schedule.data(),
        pickUpId: schedule.id,
      };
    });
    return scheduleData;
  } catch (e) {
    throw new Error(`Failed to fetch ${type} scedules: ` + error.message);
  }
};
